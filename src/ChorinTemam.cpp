...
void ChorinTemam::assemble_system_velocity()
{
    ...
    for (const auto &cell : dof_handler_velocity.active_cell_iterators())
    {
        ...
        for (unsigned int q = 0; q < n_q; ++q)
        {
            ...
            const double u_star_div = 2 * old_div[q] - old_old_div[q];
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                ...
                double rhs_contribution = scalar_product((4.0 * old_val[q] - old_old_val[q]), phi_i);
                ...
                cell_rhs(i) += rhs_contribution * JxW; // !!!! segno + o - ??
            }
        }
...
void ChorinTemam::solve_velocity_system()
{
...
void ChorinTemam::assemble_system_pressure()
{
        ...
        for (unsigned int q = 0; q < n_q; ++q)
        {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                ...
                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                    ...
                    cell_matrix(i, j) += scalar_product(grad_i, grad_j) * JxW;
                }
                // RHS = - 1/dt * div(u^*) * phi_i
                cell_rhs(i) += -dt_inv * (div_u_star[q] * phi_i) * JxW;
            }
        }
...
void ChorinTemam::solve_pressure_system()
{
...
void ChorinTemam::update_velocity()
{
    ...
    std::vector<Tensor<1, dim>> u_star_vals(n_q);
    std::vector<Tensor<1, dim>> grad_p_vals(n_q);
    ...
    for (; cell_v != end_v; ++cell_v, ++cell_p)
    {
        ...
        vel_extract.get_function_values(velocity_solution, u_star_vals);

        fe_values_p.get_function_gradients(pressure_solution, grad_p_vals);
        ...
        for (unsigned int q = 0; q < n_q; ++q)
        {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                ...
                double rhs_contrib = (u_star_vals[q] * phi_i - deltat * (grad_p_vals[q] * phi_i));
...
void ChorinTemam::solve_update_velocity_system()
{
...
void ChorinTemam::output_results()
{
...
void ChorinTemam::compute_lift_drag()
{
    // -------------------------------------------------
...
std::string ChorinTemam::get_output_directory()
{
    ...
    if (!fs::exists("./outputs/chorinTemam"))
    {
        fs::create_directory("./outputs/chorinTemam");
    }

    std::string sub_dir_name = "outputs_reynolds_" + std::to_string(static_cast<int>(reynolds_number));
    fs::path sub_dir_path = "./outputs/chorinTemam/" + sub_dir_name + "/";
